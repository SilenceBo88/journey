...
<div class="panle panel-info">
    ...
    <div class="panel-body">
        <table class="table table-striped table-hover">
            ...
            <%--显示数据--%>
            <tbody>
            <c:forEach items="${pageInfo.pageList }" var="order">
                <tr>
                    <td>${order.id }</td>
                    <td>${order.u_name }</td>
                    <td>${order.u_phone }</td>
            ...
            <tr>
                <td colspan="8">
                    <ul class="pagination">
                        <li><a href="${APP_PATH}/Order/all">首页</a></li>
                        <c:if test="${pageInfo.pageIndex > 1 }">
                            <li><a href="${APP_PATH}/Order/all?page=${pageInfo.pageIndex - 1 }">«</a></li>
                        </c:if>
                        ...
                        <c:forEach var="i" begin="${begin}" end="${end}">
                            <c:choose>
                                <c:when test="${i eq pageInfo.pageIndex }">
                                    <li class="active"><a href="${APP_PATH}/Order/all?page=${i }">${i }</a></li>
                                </c:when>
                                <c:otherwise>
                                    <li><a href="${APP_PATH}/Order/all?page=${i }">${i }</a></li>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        ...
                        <li><a href="${APP_PATH}/Order/all?page=${pageInfo.totalPage }">尾页</a></li>
                    </ul>
                </td>
            </tr>
            </tfoot>
