...
<div class="panle panel-info">
    <div class="panel-heading">
        <a class="panel-title">团队修改</a>
    </div>
    <div class="container">
        <form class="form-horizontal" action="${APP_PATH }/Team/update" method="post" enctype="multipart/form-data">
            <input type="hidden" name="team.id" value="${team.id }">
            <div class="form-group form-group-lg">
                <label for="teamName" class="col-sm-2 control-label">团队名称</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" id="teamName" name="team.teamName" value="${team.teamName }">
                </div>
            </div>
            <div class="form-group form-group-lg">
                <label for="maxNum" class="col-sm-2 control-label">人数上限</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" id="maxNum" name="team.maxNum" value="${team.maxNum }">
                </div>
            </div>
            <div class="form-group form-group-lg">
                <label for="status" class="col-sm-2 control-label">团队状态</label>
                <div class="col-sm-4">
                    <%--<input type="text" class="form-control" id="status" name="team.status" value="${team.status }">--%>
                    <select class="form-control" id="status" name="team.status">
                        <option value="0" <c:if test="${team.status eq 0}">selected</c:if>>未出发</option>
                        <option value="1" <c:if test="${team.status eq 1}">selected</c:if>>已出发</option>
                        <option value="2" <c:if test="${team.status eq 2}">selected</c:if>>已结束</option>
                    </select>
                </div>
            </div>
            <div class="form-group form-group-lg">
                <label for="tgid" class="col-sm-2 control-label">选择导游</label>
                <div class="col-sm-4">
                    <select class="form-control" id="tgid" name="team.tgid">
                        <c:forEach items="${tourGuides }" var="tg">
                            <option value="${tg.id }" <c:if test="${tg.id eq team.tgid}">selected</c:if>>${tg.realName }</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
            <div class="form-group form-group-lg">
                ...
                <div class="input-group date form_date col-sm-4" data-date-format="yyyy-mm-dd" data-link-field="dtp_input" data-link-format="yyyy-mm-dd">
                    <input class="form-control" id="goTime" type="text" name="team.goTime" placeholder="线路出发时间" readonly value="${team.goTime}">
